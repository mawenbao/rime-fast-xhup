...
local function Date2LunarDate(Gregorian)
	...
	if string.len(Gregorian) < 8 then
		return "无效日期", "无效日期"
	end
	Year  = tonumber(Gregorian.sub(Gregorian, 1, 4)) or 0
	Month = tonumber(Gregorian.sub(Gregorian, 5, 6)) or 0
	Day   = tonumber(Gregorian.sub(Gregorian, 7, 8)) or 0
	if
        (Year > 2100) or (Year < 1899)
	...
	for i = 1, _nis:len() do
		...
		_LunarYears = _LunarYears .. number_to_cn.convert_arab_to_chinese(_ni_digit)
	end
