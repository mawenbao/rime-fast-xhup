...
function E:config_path_to_str_list(path)
	return List(self:Config_get(path)):map(function(elm)
		return elm.path .. ": " .. elm.value
	end)
end
...
function E:Get_tags(ns)
    ns = ns or self.name_space or ""
    local vals = self:Config_get(ns .. "/tags")
    return vals and Set(vals) or nil
end

function E:append_value_before(path, elm)
	local obj = self:Config_get(path)
	if type(obj) ~= "table" or #obj < 1 then
		return
	end
